	...
	public List<Praticien> getPraticiens() throws SQLException {
		...
		try {
			...
			String requete = "SELECT PRA_NUM, PRA_NOM, PRA_PRENOM, PRA_ADRESSE, PRA_CP, PRA_VILLE, "
					+ "PRA_COEFNOTORIETE, TYP_CODE, RAP_DATE_VISITE, COEF_CODE "
					+ "FROM PRATICIEN INNER JOIN RAPPORT_VISITE "
					+ "ON PRATICIEN.PRA_NUM = RAPPORT_VISITE.PRA_NUM" ;
			...
			while(resultat.next()) {
				...
				String dateDerniereVisite = resultat.getString("RAP_DATE_VISITE") ;
				int coefConfiance = resultat.getInt("COEF_CODE") ;
				
				lesPraticiens.add(new Praticien(praNum, praNom, praPrenom, praAdresse, praCP, 
						praVille, praCoefNotoriete, typCode, dateDerniereVisite, coefConfiance)) ;
